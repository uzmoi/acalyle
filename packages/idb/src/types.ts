...
type IDBRequestResult<T> =
    T extends IDBCursor ? IdbCursor<unknown, IDBTransactionMode>
    :   Promise<unknown>;

type IDBResult<T> =
    T extends IDBRequest<infer U> ? IDBRequestResult<U>
    : T extends IDBTransaction ?
        IdbTransaction<Record<string, IdbObjectStoreSchema>, IDBTransactionMode>
    : T extends IDBIndex ? IdbIndex<unknown, IDBTransactionMode>
    : T;
