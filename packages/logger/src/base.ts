...
export abstract class Logger<
  in Message,
  in Level,
  in MetaData = undefined,
> implements Transport<Message, Level, MetaData> {
  constructor(protected readonly name: string) {}
  private readonly _transports = new Set<Transport<Message, Level, MetaData>>();
  ...
  protected log(
    level: Level,
    message: Message,
    options?: Partial<Omit<Log<Message, Level, MetaData>, "level" | "message">>,
  ): void {
    const { meta, timeStamp = Date.now(), name = this.name } = options ?? {};
